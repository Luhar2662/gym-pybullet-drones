...
pre-tuned controllers), and filter is implemented using an ISAACS critic.

Filter style: simple switching. When q_val prediction falls below a given threshold, the fallback actor
is activated (guard mode) and is prioritized for a set guard duration before returning control to PID.

SafeHoverAviary observations are reconstructed from CtrlAviary state to give valid observations to the critic.
Disturbance inputs to the ISAACS critic are sampled from the trained disturbance actor.

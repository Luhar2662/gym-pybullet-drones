'''
Evaluation script for a predictive filter using an ISAACS trained model. 

Runs num_runs episodes (in parallel across num_workers processes), rolls out full episodes, and
reports rates of success and safety violation across episodes.
...
def safe_random_init(model, threshold):
    ...
    while not verified:
        ...
        dist_t = th.zeros((1,) + model.disturbance_space.shape, device=model.device)
        with th.no_grad():
...
def predictive_safety_check(
    ...
    '''
    Rolls the environment forward `horizon` steps to check for future safety violations
    without committing the steps to the real simulation.

    Step 0 uses `candidate_norm`; steps 1+ use the ISAACS fallback policy.
    At each future state:
      - check_bounds: explicit safety margin <= 0
      - check_q: Q(s', pi_fallback(s'), d) < threshold

    Works with both raw CtrlAviary and DisturbedEnvWrapper-wrapped envs:
      - env.step() is called on the outer env so disturbances are applied in the rollout
      - PyBullet state save/restore is done via env.unwrapped

    Returns (safe: bool, failure_type: str)
    '''
    if horizon == 0:
        return True, ""

    raw_env = env.unwrapped
    p_client = raw_env.getPyBulletClient()

    # Save env state
    saved_pyb_state = p.saveState(physicsClientId=p_client)
    saved_step_counter = raw_env.step_counter
    saved_last_clipped = raw_env.last_clipped_action.copy()
    ...
    try:
        for step_k in range(horizon):
            ...
            raw_env.step(action_rpm)

            new_state = raw_env._getDroneStateVector(0)
            sim_buffer.append(current_action_norm.reshape(1, 4).astype(np.float32))
            sim_obs = build_safe_obs(new_state, sim_buffer)
            ...
            # Check Q-value of fallback policy at the future state
            if check_q:
                ...
                with th.no_grad():
                    ...
                    q1, q2 = model.policy.critic(obs_t, act_t, dist_t)
                q_mean = np.min([q1.item(), q2.item()])
                if q_mean < threshold:
                    violated = True
                    failure_type = f"q_violation at step +{step_k + 1} (q={q_mean:.4f})"
                    break

            # Next rollout step uses ISAACS fallback
            if step_k < horizon - 1:
        ...
        raw_env.step_counter = saved_step_counter
        raw_env.last_clipped_action = saved_last_clipped
        raw_env._updateAndStoreKinematicInformation()
...
def run_episode(env, model, ctrl, HOVER_RPM, target, start_pos, start_rpy,
                ...
                safety_threshold, seed, horizon, check_q, check_bounds):
    ...
    for i in range(int(duration_sec * env.CTRL_FREQ)):
        ...
        if not guard_active:
            if horizon > 0:
                # Predictive check: roll forward `horizon` steps
                safe, _ = predictive_safety_check(
                    env=env,
                    model=model,
                ...
                # horizon=0: standard one-step Q check (same as ISAACS_deep_eval_switching.py)
                obs_t, _ = model.policy.obs_to_tensor(safe_obs)
                act_t = th.as_tensor(norm_candidate, device=model.device).float()
                dist_t = th.zeros((1,) + model.disturbance_space.shape, device=model.device)
                with th.no_grad():
                    q1, q2 = model.policy.critic(obs_t, act_t, dist_t)
                if np.mean([q1.item(), q2.item()]) < safety_threshold:
                    guard_active = True
                    guard_timesteps = guard_duration
                    guard_activated += 1
...
def _worker_init(model_path, start_pos, start_rpy, drone, physics, sim_freq, disturbed, disturbance_bound):
    ...
    _worker_model = ISAACS.load(model_path)
...
def _worker_run(task):
    seed, target, duration_sec, num_segments, segment_path, guard_duration, safety_threshold, horizon, check_q, check_bounds = task
    return run_episode(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--horizon',            default=DEFAULT_HORIZON,            type=int,        metavar='', help='Predictive rollout horizon (0 = standard one-step Q check)')
    parser.add_argument('--check_q',            default=DEFAULT_CHECK_Q,            type=str2bool,   metavar='', help='Check Q-value of fallback policy at each future rollout state')
    parser.add_argument('--check_bounds',       default=DEFAULT_CHECK_BOUNDS,       type=str2bool,   metavar='', help='Check explicit safety bound violation at each future rollout state')
    ...
    # Load model in main process  to generate targets first
    model = ISAACS.load(ARGS.model_path)
    ...
    tasks = [
        (i, targets[i], ARGS.duration_sec, ARGS.num_segments, ARGS.segment_path,
         ARGS.guard_duration, ARGS.safety_threshold, ARGS.horizon, ARGS.check_q, ARGS.check_bounds)
        for i in range(ARGS.num_runs)
    ]

    print(f"[INFO] Predictive filter: horizon={ARGS.horizon}, check_q={ARGS.check_q}, check_bounds={ARGS.check_bounds}")
    ...
    #Collect answers from seperate envs
